...
class Ps2SettingsActivity : Activity() {
    ...
    private fun render(){
        ...
        addButton(root,"RESET RECOMMENDED Z9X") { profile=Ps2Settings.preset("Auto Z9x");save() }
        addButton(root,"PS2 BIOS / DIAGNOSTIC") { startActivity(Intent(this,Ps2BiosActivity::class.java)) }
        root.addView(TextView(this).apply{text="Perubahan diterapkan saat game PS2 berikutnya dibuka. Cycle Skip dapat menyebabkan stutter; biarkan Off kecuali diperlukan.";textSize=11f;setTextColor(0xFF777777.toInt());setPadding(0,dp(16),0,0)})
    ...
    private fun updateSummary(){if(::summary.isInitialized)summary.text="${profile.preset} • ${profile.upscale}× • EE ${profile.eeRate} • Skip ${profile.eeSkip} • MTVU ${if(profile.mtvu)"ON" else "OFF"} • ${if(profile.affinity==7)"Perf Cores" else "Auto CPU"}"}
}
