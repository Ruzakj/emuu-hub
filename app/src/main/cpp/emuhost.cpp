...
static void* hwProc(const char* name){return reinterpret_cast<void*>(eglGetProcAddress(name));}
...
static bool createHw(retro_hw_render_callback* cb){
    if(!cb)return false;
    if(cb->context_type!=RETRO_HW_CONTEXT_OPENGLES2&&cb->context_type!=RETRO_HW_CONTEXT_OPENGLES3&&cb->context_type!=RETRO_HW_CONTEXT_OPENGLES_VERSION){
        LOGE("Unsupported HW context type %d",(int)cb->context_type);return false;
    }
    destroyHw();
    eglDisplay=eglGetDisplay(EGL_DEFAULT_DISPLAY);if(eglDisplay==EGL_NO_DISPLAY||!eglInitialize(eglDisplay,nullptr,nullptr))return false;
    const EGLint cfgAttrs[]={EGL_SURFACE_TYPE,EGL_PBUFFER_BIT,EGL_RENDERABLE_TYPE,EGL_OPENGL_ES2_BIT|0x0040,EGL_RED_SIZE,8,EGL_GREEN_SIZE,8,EGL_BLUE_SIZE,8,EGL_ALPHA_SIZE,8,EGL_DEPTH_SIZE,cb->depth?24:0,EGL_STENCIL_SIZE,cb->stencil?8:0,EGL_NONE};
    EGLConfig cfg=nullptr;EGLint count=0;if(!eglChooseConfig(eglDisplay,cfgAttrs,&cfg,1,&count)||count<1){destroyHw();return false;}
    const EGLint pbAttrs[]={EGL_WIDTH,2048,EGL_HEIGHT,2048,EGL_NONE};eglSurface=eglCreatePbufferSurface(eglDisplay,cfg,pbAttrs);if(eglSurface==EGL_NO_SURFACE){destroyHw();return false;}
    unsigned major=cb->version_major;if(cb->context_type==RETRO_HW_CONTEXT_OPENGLES3&&major<3)major=3;if(major<2)major=2;
    const EGLint ctxAttrs[]={EGL_CONTEXT_CLIENT_VERSION,(EGLint)major,EGL_NONE};eglBindAPI(EGL_OPENGL_ES_API);eglContext=eglCreateContext(eglDisplay,cfg,EGL_NO_CONTEXT,ctxAttrs);if(eglContext==EGL_NO_CONTEXT){destroyHw();return false;}
    ...
    eglMakeCurrent(eglDisplay,EGL_NO_SURFACE,EGL_NO_SURFACE,EGL_NO_CONTEXT);
    LOGI("EGL hardware context ready: GLES %u.%u",major,cb->version_minor);return true;
}
...
static bool envCb(unsigned cmd,void* data){
    switch(cmd){
        ...
        case ENV_SET_SYSTEM_AV_INFO:{auto*av=reinterpret_cast<retro_system_av_info*>(data);if(av){frameW=av->geometry.base_width;frameH=av->geometry.base_height;sampleRate=(int)av->timing.sample_rate;}return true;}
...
static void captureHw(unsigned width,unsigned height){
    ...
    frameW=width;frameH=height;std::vector<uint8_t> rgba((size_t)width*height*4);glFinish();glReadPixels(0,0,width,height,GL_RGBA,GL_UNSIGNED_BYTE,rgba.data());frame.resize((size_t)width*height);
    for(unsigned y=0;y<height;y++){unsigned sy=hwBottomLeft?(height-1-y):y;for(unsigned x=0;x<width;x++){size_t s=((size_t)sy*width+x)*4;frame[(size_t)y*width+x]=0xFF000000u|((uint32_t)rgba[s]<<16)|((uint32_t)rgba[s+1]<<8)|rgba[s+2];}}
}
...
extern "C" JNIEXPORT jboolean JNICALL Java_com_ric_emuhub_core_NativeBridge_init(JNIEnv*e,jobject,jstring c,jstring s,jstring v){const char*cp=e->GetStringUTFChars(c,nullptr),*sp=e->GetStringUTFChars(s,nullptr),*sv=e->GetStringUTFChars(v,nullptr);const bool wantsAnalog=std::strstr(cp,"pcsx")||std::strstr(cp,"ppsspp");systemDir=sp;saveDir=sv;core=dlopen(cp,RTLD_NOW|RTLD_LOCAL);e->ReleaseStringUTFChars(c,cp);e->ReleaseStringUTFChars(s,sp);e->ReleaseStringUTFChars(v,sv);if(!core){LOGE("dlopen: %s",dlerror());return JNI_FALSE;}if(!sym(p_retro_init,"retro_init")||!sym(p_retro_deinit,"retro_deinit")||!sym(p_retro_set_environment,"retro_set_environment")||!sym(p_retro_set_video_refresh,"retro_set_video_refresh")||!sym(p_retro_set_audio_sample,"retro_set_audio_sample")||!sym(p_retro_set_audio_sample_batch,"retro_set_audio_sample_batch")||!sym(p_retro_set_input_poll,"retro_set_input_poll")||!sym(p_retro_set_input_state,"retro_set_input_state")||!sym(p_retro_load_game,"retro_load_game")||!sym(p_retro_unload_game,"retro_unload_game")||!sym(p_retro_run,"retro_run")||!sym(p_retro_reset,"retro_reset")||!sym(p_retro_get_system_av_info,"retro_get_system_av_info"))return JNI_FALSE;opt(p_retro_set_controller_port_device,"retro_set_controller_port_device");opt(p_retro_serialize_size,"retro_serialize_size");opt(p_retro_serialize,"retro_serialize");opt(p_retro_unserialize,"retro_unserialize");p_retro_set_environment(envCb);p_retro_set_video_refresh(videoCb);p_retro_set_audio_sample(audioCb);p_retro_set_audio_sample_batch(audioBatchCb);p_retro_set_input_poll(inputPollCb);p_retro_set_input_state(inputStateCb);p_retro_init();if(p_retro_set_controller_port_device)p_retro_set_controller_port_device(0,wantsAnalog?RETRO_DEVICE_ANALOG:RETRO_DEVICE_JOYPAD);return JNI_TRUE;}
...
extern "C" JNIEXPORT jboolean JNICALL Java_com_ric_emuhub_core_NativeBridge_loadGame(JNIEnv*e,jobject,jstring path){const char*p=e->GetStringUTFChars(path,nullptr);retro_game_info info{p,nullptr,0,nullptr};bool ok=p_retro_load_game&&p_retro_load_game(&info);e->ReleaseStringUTFChars(path,p);if(ok&&hwEnabled&&!hwResetCalled&&hwContextReset&&makeHwCurrent()){hwContextReset();hwResetCalled=true;}if(ok&&p_retro_get_system_av_info){retro_system_av_info av{};p_retro_get_system_av_info(&av);frameW=av.geometry.base_width;frameH=av.geometry.base_height;sampleRate=(int)(av.timing.sample_rate>0?av.timing.sample_rate:44100);frame.resize((size_t)frameW*frameH);audioBuffer.clear();}return ok?JNI_TRUE:JNI_FALSE;}
extern "C" JNIEXPORT jint JNICALL Java_com_ric_emuhub_core_NativeBridge_runFrame(JNIEnv*e,jobject,jintArray p){if(!p_retro_run)return 0;if(hwEnabled)makeHwCurrent();p_retro_run();jsize n=e->GetArrayLength(p);size_t c=std::min(frame.size(),(size_t)n);if(c)e->SetIntArrayRegion(p,0,(jsize)c,(const jint*)frame.data());return(jint)c;}
