...
static bool makeHwCurrent(){
    return hwEnabled && eglDisplay!=EGL_NO_DISPLAY && eglSurface!=EGL_NO_SURFACE &&
           eglContext!=EGL_NO_CONTEXT && eglMakeCurrent(eglDisplay,eglSurface,eglSurface,eglContext)==EGL_TRUE;
}

// The pbuffer default framebuffer is the frontend render target.
static uintptr_t hwFramebuffer(){ return 0; }

static void* hwProc(const char* name){
...
static void destroyHw(){
    if(eglDisplay!=EGL_NO_DISPLAY){
        if(eglContext!=EGL_NO_CONTEXT && hwContextDestroy && makeHwCurrent()) hwContextDestroy();
    ...
    hwEnabled=false;
    hwResetCalled=false;
    hwContextReset=nullptr;
    hwContextDestroy=nullptr;
}
...
static bool createHw(retro_hw_render_callback* cb){
    ...
    if(cb->context_type!=RETRO_HW_CONTEXT_OPENGLES2 &&
       ...
       cb->context_type!=RETRO_HW_CONTEXT_OPENGLES_VERSION){
        LOGE("Unsupported HW context type %d",(int)cb->context_type);
        return false;
    }
    ...
    if(!eglChooseConfig(eglDisplay,cfgAttrs,&cfg,1,&count) || count<1){
    ...
    if(eglContext==EGL_NO_CONTEXT){
        LOGE("eglCreateContext GLES%u failed: 0x%x",major,eglGetError());
        destroyHw(); return false;
    }

    hwEnabled=true;
    hwBottomLeft=cb->bottom_left_origin;
    hwContextReset=cb->context_reset;
    hwContextDestroy=cb->context_destroy;
    cb->get_current_framebuffer=hwFramebuffer;
    cb->get_proc_address=hwProc;

    // Libretro HW cores expect context_reset after the frontend context exists,
    // before game loading/render initialization. Waiting until after load_game
    // leaves PPSSPP's renderer uninitialized and produces a permanent black frame.
    if(!makeHwCurrent()){
    ...
    glFinish();
    LOGI("HW context ready before load: GLES %u.%u, reset=%d",major,cb->version_minor,hwResetCalled?1:0);
    return true;
}

static const char* ppssppOption(const char* key){
    if(!key) return nullptr;
    // Current and legacy PPSSPP-libretro keys. Keep conservative defaults for
    // demanding games: GLES/OpenGL renderer, native PSP resolution and JIT.
    if(std::strcmp(key,"ppsspp_backend")==0) return "opengl";
    if(std::strcmp(key,"ppsspp_rendering_mode")==0) return "OpenGL";
    if(std::strcmp(key,"ppsspp_internal_resolution")==0) return "480x272";
    if(std::strcmp(key,"ppsspp_cpu_core")==0) return "JIT";
    if(std::strcmp(key,"ppsspp_frameskip")==0) return "0";
    if(std::strcmp(key,"ppsspp_auto_frameskip")==0) return "disabled";
    ...
    if(std::strcmp(key,"ppsspp_texture_scaling_level")==0) return "1";
    return nullptr;
}
...
static bool envCb(unsigned cmd,void* data){
    switch(cmd){
        case ENV_GET_CAN_DUPE:
            *reinterpret_cast<bool*>(data)=true; return true;
        case ENV_GET_OVERSCAN:
            *reinterpret_cast<bool*>(data)=false; return true;
        case ENV_GET_SYSTEM_DIRECTORY:
            *reinterpret_cast<const char**>(data)=systemDir.c_str(); return true;
        case ENV_GET_SAVE_DIRECTORY:
            *reinterpret_cast<const char**>(data)=saveDir.c_str(); return true;
        case ENV_SET_PIXEL_FORMAT:
            pixelFormat=*reinterpret_cast<const int*>(data); return true;
        case ENV_GET_VARIABLE_UPDATE:
            *reinterpret_cast<bool*>(data)=false; return true;
        case ENV_GET_VARIABLE: {
            auto* v=reinterpret_cast<retro_variable*>(data);
            if(!v) return false;
            v->value=isPpsspp?ppssppOption(v->key):nullptr;
            if(v->value){ LOGI("Core option %s=%s",v->key,v->value); return true; }
            return false;
        }
        case ENV_GET_LOG_INTERFACE: {
            auto* l=reinterpret_cast<retro_log_callback*>(data); l->log=coreLog; return true;
        }
        case ENV_GET_LANGUAGE:
            *reinterpret_cast<unsigned*>(data)=0; return true;
        case ENV_GET_INPUT_BITMASKS:
            return true;
        case ENV_GET_PREFERRED_HW_RENDER:
            *reinterpret_cast<unsigned*>(data)=RETRO_HW_CONTEXT_OPENGLES3; return true;
        case ENV_SET_HW_RENDER:
            return createHw(reinterpret_cast<retro_hw_render_callback*>(data));
        case ENV_SET_SYSTEM_AV_INFO: {
            ...
            if(av){
                ...
                sampleRate=(int)(av->timing.sample_rate>0?av->timing.sample_rate:44100);
            }
            return true;
        }
        case ENV_SET_GEOMETRY: {
            auto* g=reinterpret_cast<retro_game_geometry*>(data);
            if(g){frameW=g->base_width;frameH=g->base_height;}
            return true;
        }
...
static void captureHw(unsigned width,unsigned height){
    ...
    // The libretro callback means the frame is in the frontend framebuffer.
    // PPSSPP may leave one of its private FBOs bound, so explicitly read our
    // pbuffer default framebuffer instead of whatever happens to be current.
    GLint previousFbo=0;
    glGetIntegerv(GL_FRAMEBUFFER_BINDING,&previousFbo);
    glBindFramebuffer(GL_FRAMEBUFFER,0);
    glFinish();
    glPixelStorei(GL_PACK_ALIGNMENT,1);
    glReadPixels(0,0,(GLsizei)width,(GLsizei)height,GL_RGBA,GL_UNSIGNED_BYTE,rgba.data());
    GLenum err=glGetError();
    glBindFramebuffer(GL_FRAMEBUFFER,(GLuint)previousFbo);
    if(err!=GL_NO_ERROR){
        LOGE("glReadPixels failed: 0x%x, %ux%u",err,width,height);
        return;
    }

    frame.resize((size_t)width*height);
    for(unsigned y=0;y<height;y++){
        unsigned sy=hwBottomLeft?(height-1-y):y;
        for(unsigned x=0;x<width;x++){
            size_t s=((size_t)sy*width+x)*4;
            frame[(size_t)y*width+x]=0xFF000000u|((uint32_t)rgba[s]<<16)|((uint32_t)rgba[s+1]<<8)|rgba[s+2];
        }
    }
}
...
static int16_t inputStateCb(unsigned port,unsigned device,unsigned index,unsigned id){
    ...
    if(device==RETRO_DEVICE_JOYPAD && index==0){
        if(id==RETRO_DEVICE_ID_JOYPAD_MASK){
            uint16_t mask=0; for(unsigned i=0;i<16;i++) if(buttons[i]) mask|=(uint16_t)(1u<<i);
            return (int16_t)mask;
        }
...
template<typename T> static bool sym(T&out,const char*n){
    out=reinterpret_cast<T>(dlsym(core,n));
    if(!out){LOGE("Missing symbol %s",n);return false;}
    return true;
}
...
extern "C" JNIEXPORT jboolean JNICALL Java_com_ric_emuhub_core_NativeBridge_init(JNIEnv*e,jobject,jstring c,jstring s,jstring v){
    const char*cp=e->GetStringUTFChars(c,nullptr);
    const char*sp=e->GetStringUTFChars(s,nullptr);
    const char*sv=e->GetStringUTFChars(v,nullptr);
    isPpsspp=std::strstr(cp,"ppsspp")!=nullptr;
    const bool wantsAnalog=std::strstr(cp,"pcsx")!=nullptr || isPpsspp;
    ...
    if(!sym(p_retro_init,"retro_init")||!sym(p_retro_deinit,"retro_deinit")||
       !sym(p_retro_set_environment,"retro_set_environment")||!sym(p_retro_set_video_refresh,"retro_set_video_refresh")||
       !sym(p_retro_set_audio_sample,"retro_set_audio_sample")||!sym(p_retro_set_audio_sample_batch,"retro_set_audio_sample_batch")||
       !sym(p_retro_set_input_poll,"retro_set_input_poll")||!sym(p_retro_set_input_state,"retro_set_input_state")||
       !sym(p_retro_load_game,"retro_load_game")||!sym(p_retro_unload_game,"retro_unload_game")||
       !sym(p_retro_run,"retro_run")||!sym(p_retro_reset,"retro_reset")||
    ...
    opt(p_retro_serialize_size,"retro_serialize_size");
    opt(p_retro_serialize,"retro_serialize");
    opt(p_retro_unserialize,"retro_unserialize");
    ...
    LOGI("Core initialized: ppsspp=%d hw=%d reset=%d",isPpsspp?1:0,hwEnabled?1:0,hwResetCalled?1:0);
    return JNI_TRUE;
}

extern "C" JNIEXPORT void JNICALL Java_com_ric_emuhub_core_NativeBridge_setControllerDevice(JNIEnv*,jobject,jint device){
    if(p_retro_set_controller_port_device) p_retro_set_controller_port_device(0,device==RETRO_DEVICE_ANALOG?RETRO_DEVICE_ANALOG:RETRO_DEVICE_JOYPAD);
}

extern "C" JNIEXPORT jboolean JNICALL Java_com_ric_emuhub_core_NativeBridge_loadGame(JNIEnv*e,jobject,jstring path){
    if(hwEnabled && !makeHwCurrent()) LOGE("HW context not current before load_game");
    ...
    // Compatibility fallback only; the correct reset happens in createHw().
    if(ok && hwEnabled && !hwResetCalled && hwContextReset && makeHwCurrent()){
        hwContextReset(); hwResetCalled=true;
    }
    if(ok && p_retro_get_system_av_info){
        retro_system_av_info av{}; p_retro_get_system_av_info(&av);
        frameW=av.geometry.base_width; frameH=av.geometry.base_height;
        sampleRate=(int)(av.timing.sample_rate>0?av.timing.sample_rate:44100);
        frame.assign((size_t)frameW*frameH,0xFF000000u);
        audioBuffer.clear();
        LOGI("Game loaded: %ux%u %.2ffps %dHz hw=%d",frameW,frameH,av.timing.fps,sampleRate,hwEnabled?1:0);
    }
...
extern "C" JNIEXPORT jint JNICALL Java_com_ric_emuhub_core_NativeBridge_runFrame(JNIEnv*e,jobject,jintArray p){
    ...
    if(hwEnabled && !makeHwCurrent()) LOGE("eglMakeCurrent failed before retro_run: 0x%x",eglGetError());
    p_retro_run();
    jsize n=e->GetArrayLength(p);
    size_t c=std::min(frame.size(),(size_t)n);
    if(c) e->SetIntArrayRegion(p,0,(jsize)c,(const jint*)frame.data());
    return (jint)c;
}
...
extern "C" JNIEXPORT jint JNICALL Java_com_ric_emuhub_core_NativeBridge_readAudio(JNIEnv*e,jobject,jshortArray out){
    if(!out||audioBuffer.empty()) return 0;
    size_t c=std::min((size_t)e->GetArrayLength(out),audioBuffer.size());
    e->SetShortArrayRegion(out,0,(jsize)c,(const jshort*)audioBuffer.data());
    audioBuffer.erase(audioBuffer.begin(),audioBuffer.begin()+(long)c);
    return (jint)c;
}

extern "C" JNIEXPORT jboolean JNICALL Java_com_ric_emuhub_core_NativeBridge_saveState(JNIEnv*e,jobject,jstring path){
    if(!p_retro_serialize_size||!p_retro_serialize) return JNI_FALSE;
    if(hwEnabled) makeHwCurrent();
    size_t z=p_retro_serialize_size(); if(!z) return JNI_FALSE;
    std::vector<uint8_t>s(z); if(!p_retro_serialize(s.data(),z)) return JNI_FALSE;
    const char*p=e->GetStringUTFChars(path,nullptr);
    std::ofstream o(p,std::ios::binary|std::ios::trunc); e->ReleaseStringUTFChars(path,p);
    if(!o) return JNI_FALSE; o.write((const char*)s.data(),s.size()); return o.good()?JNI_TRUE:JNI_FALSE;
}

extern "C" JNIEXPORT jboolean JNICALL Java_com_ric_emuhub_core_NativeBridge_loadState(JNIEnv*e,jobject,jstring path){
    if(!p_retro_unserialize) return JNI_FALSE;
    if(hwEnabled) makeHwCurrent();
    const char*p=e->GetStringUTFChars(path,nullptr);
    std::ifstream in(p,std::ios::binary|std::ios::ate); e->ReleaseStringUTFChars(path,p);
    if(!in) return JNI_FALSE; auto z=in.tellg(); if(z<=0) return JNI_FALSE;
    in.seekg(0); std::vector<uint8_t>s((size_t)z); if(!in.read((char*)s.data(),z)) return JNI_FALSE;
    audioBuffer.clear(); return p_retro_unserialize(s.data(),s.size())?JNI_TRUE:JNI_FALSE;
}
...
extern "C" JNIEXPORT void JNICALL Java_com_ric_emuhub_core_NativeBridge_reset(JNIEnv*,jobject){if(hwEnabled)makeHwCurrent();if(p_retro_reset)p_retro_reset();}
...
extern "C" JNIEXPORT void JNICALL Java_com_ric_emuhub_core_NativeBridge_unload(JNIEnv*,jobject){
    ...
    core=nullptr; isPpsspp=false;
    frame.clear(); audioBuffer.clear();
    std::fill(std::begin(buttons),std::end(buttons),false);
    analogX=analogY=0;
    p_retro_set_controller_port_device=nullptr;
}
